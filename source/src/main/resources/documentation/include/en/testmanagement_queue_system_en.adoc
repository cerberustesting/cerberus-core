...
- If you want to increase the priority of the given execution, you can change it to any value bellow 100. The lower priority value takes, the higher the priority will be.
- If you want some execution to be treated last, change it to any value above 100.
...
- constrain **'constrain1_global'** : Global level of constrain in order to secure that Cerberus itself is not saturated by number of simultaneous execution.
 Parameter 'cerberus_queueexecution_global_threadpoolsize' allow to configure the number of simultaneous execution will handle in total.
...
- constrain **'constrain3_robot'** : Robot level constrain. Some robot cannot handle too many execution at the same time. The configuration of simultaneous xecution is done at robot host level.
