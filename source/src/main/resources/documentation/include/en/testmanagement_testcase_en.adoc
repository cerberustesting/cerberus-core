...
Aditionaly, Cerberus will add a `stat` json entry with the following format. That section will be feed by checking every hit performed by the application. Every hit will enrish the `total` section but also one of the following section : either `INTERNAL`, one `ThridPartyN` or `UNKNOWN` if the hit cannot match one of the application domain or one of the Third Party domain.
...
Cerberus will get and guess the `ThirdPartyN` from a json file that can be retreive from the following https://github.com/patrickhulce/third-party-web/blob/master/data/entities.json5[repo].
