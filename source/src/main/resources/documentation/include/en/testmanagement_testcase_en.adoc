...
*[blue]#Value2#*	Arguments of the command.
...
Syntax in order to identify a field or element inside a page. Syntax has the form :
...
Additionally, Cerberus will add a `stat` json entry with the following format. That section will be feed by checking every hit performed by the application. Every hit will enrish the `total` section but also one of the following section : either `internal`, either one of `ThirdPartyN` inside `thirdparty` or `unknown` if the hit cannot match one of the application domain or one of the Third Party domain.
    ...
    "stat": {
        "total":{
            ...
            "httpReq":{
                "nbRequests": 117,
                "urlError": [ "https://www.laredoute.com/mar/to.jpg"],
                "nbError": 1,
                "nb200": 111,
                "nb300": 0,
                "nb301": 2,
            ...
            "type":{
                "css":{
                    ...
                    "url": [ "https://www.laredoute.com/mar/13.css" ]
                },
                "img":{...},
                "other":{...},
                "js":{...},
...
Cerberus will get and guess the `ThirdPartyN` from a json file that can be retrieve from the following https://github.com/patrickhulce/third-party-web/blob/master/data/entities.json5[repo].
...
`internal` is determined from the URL provided by the corresponding domain list defined on the corresponding application / environment. all hosts should be defined on the domain field with coma separated format.

Any host URL that does not patch either the ThirdParty definition file or list of application domain will appear in `unknown` section.
