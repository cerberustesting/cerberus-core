...
/* =============================================================================
 * Environment list - V2, on js/global/crbTable.js.
 *
 * SCOPE. Only the page's own list is migrated. The two tables INSIDE the edit
 * modal (last changes, last events) stay legacy DataTables: they are modal
 * content driven by loadChangeTable()/loadEventTable(), carried over verbatim
 * below along with every modal handler.
 *
 * Endpoint: ReadCountryEnvParam?forceList=Y (contentTable + table-level
 * hasPermissions + a columnName -> distinctValues mode for the column filters).
 *
 * Action gating, ported from Environment.js:1204-1281:
 *   - Edit/View : always rendered; icon AND tooltip swap on hasPermissions
 *   - Delete    : permission
 *   - Disable   : permission AND the environment is active
 *   - New chain : permission AND the environment is active AND chain === "Y"
 *   - Enable    : permission AND the environment is NOT active
 *
 * ONE DELIBERATE DEVIATION FROM V1, flagged rather than silently kept:
 * V1 rendered Enable / Disable / New chain OUTSIDE its `if (hasPermissions)`
 * block, so any logged-in user saw them. And the three servlets they call
 * (DisableEnvironment, NewBuildRev, NewChain) live in
 * org.cerberus.core.servlet.integration and carry NO permission check of their
 * own - they are meant to be callable from a CI pipeline. Net effect on the
 * legacy page: a read-only user can disable a production environment from the
 * UI. V2 gates the three on the permission. The CI entry points are untouched,
 * so nothing automated changes; only the UI stops offering the button to users
 * who are not supposed to press it.
 *
 * Fixed on the way over:
 *   - every handler was built as onclick="fn('"+escapeHtml(system)+"',...)".
 *     HTML-escaping does not protect a JS string inside an attribute, so a system
 *     named  x'); alert(1); //  ran. V2 passes the row to onClick.
 *   - the Active / Maintenance columns rendered a disabled <input type=checkbox>,
 *     which reads as an editable control that refuses to move. They are chips now.
 * ========================================================================== */
...
var CRB_ENV_TABLE_ID = "environmentTableV2";
...
function initPage() {
    ...
    // Invariant combos of the add/edit modals. Kept identical to V1.
    ...
    // Caches the modals read at open time.
    ...
    createCerberusTable({
        id: CRB_ENV_TABLE_ID,
        mount: "#environmentList",
        endpoint: "ReadCountryEnvParam?forceList=Y",
        // The distinct-values mode is guarded by `request.getParameter("system") != null`
        // (ReadCountryEnvParam.java:128), so the systems have to be in this URL or every
        // column filter comes back empty without saying why. Verified live.
        distinctEndpoint: "ReadCountryEnvParam?forceList=Y" + getUser().defaultSystemsQuery,
        // system + country + environment is the primary key of an environment.
        rowKey: function (row) { return JSON.stringify([row.system, row.country, row.environment]); },
        defaultSort: {field: "environment", dir: "asc"},
        pageLength: 10,
        searchPlaceholder: "Search environments...",
        emptyMessage: "No environment matches your search",
        // ?system=&country=&environment=&build=&revision=&envgp=&active= deep links,
        // applied to the FIRST request. V1 applied them after a first unfiltered load.
        initialFilters: envV2FiltersFromUrl(),

        toolbar: function (ctx) {
            if (!ctx.hasPermissions) {
                return "";
            }
            return "<button id='createEnvButton' type='button' onclick='addEntryClick()' " +
                ...
                doc.getDocLabel("page_environment", "button_create") + "</span></button>";
        },

        // `field` = server column name (sName in V1), `prop` = key in the row.
        // They differ on envGp, whose server name is inv.gp1.
        columns: [
            {field: "system", title: doc.getDocOnline("invariant", "SYSTEM"),
             width: "130px", filterable: true, className: "font-medium"},
            {field: "country", title: doc.getDocOnline("invariant", "COUNTRY"),
             width: "100px", filterable: true},
            {field: "environment", title: doc.getDocOnline("invariant", "ENVIRONMENT"),
             width: "130px", filterable: true},
            {field: "description", title: doc.getDocOnline("countryenvparam", "Description"),
             width: "300px", like: true},
            {
                field: "active", title: doc.getDocOnline("countryenvparam", "active"),
                width: "110px", filterable: true,
                render: function (row) { return envV2Flag(row.active); }
            },

            // ---- available from Config, hidden by default ----
            {field: "inv.gp1", prop: "envGp", title: doc.getDocOnline("page_environment", "envgp"),
             width: "140px", visible: false, filterable: true},
            {field: "build", title: doc.getDocOnline("buildrevisioninvariant", "versionname01"),
             width: "120px", visible: false, filterable: true},
            {field: "revision", title: doc.getDocOnline("buildrevisioninvariant", "versionname02"),
             width: "120px", visible: false, filterable: true},
            {field: "type", title: doc.getDocOnline("countryenvparam", "Type"),
             width: "110px", visible: false, filterable: true},
            {
                field: "maintenanceAct", title: doc.getDocOnline("countryenvparam", "maintenanceact"),
                width: "140px", visible: false,
                render: function (row) { return envV2Flag(row.maintenanceAct); }
            },
            {field: "maintenanceStr", title: doc.getDocOnline("countryenvparam", "maintenancestr"),
             width: "130px", visible: false},
            {field: "maintenanceEnd", title: doc.getDocOnline("countryenvparam", "maintenanceend"),
             width: "130px", visible: false},
            {field: "chain", title: doc.getDocOnline("countryenvparam", "chain"),
        ...
        actions: [
            {
                ...
                title: function (row, ctx) {
                    return doc.getDocLabel("page_environment",
                        ctx.hasPermissions ? "button_edit" : "button_view");
                },
                onClick: function (row) { editEntryClick(row.system, row.country, row.environment); }
            },
            {
                key: "delete", icon: "trash-2", gate: "permission", danger: true,
                title: doc.getDocLabel("page_environment", "button_delete"),
                onClick: function (row) { deleteEntryClick(row.system, row.country, row.environment); }
            },
            {
                // See the header note: gated on the permission, which V1 did not do.
                key: "disable", icon: "power-off",
                gate: function (row, ctx) { return Boolean(ctx.hasPermissions) && envV2IsActive(row); },
                title: doc.getDocLabel("page_environment", "button_disable"),
                onClick: function (row) { eventDisableClick(row.system, row.country, row.environment); }
            },
            {
                key: "newchain", icon: "link",
                gate: function (row, ctx) {
                    return Boolean(ctx.hasPermissions) && envV2IsActive(row) && row.chain === "Y";
                },
                title: doc.getDocLabel("page_environment", "button_newChain"),
                onClick: function (row) { eventNewChainClick(row.system, row.country, row.environment); }
            },
            {
                ...
                onClick: function (row) {
                    eventEnableClick(row.system, row.country, row.environment, row.build, row.revision);
                }
            }
        ]
    });

    // Modal save buttons. Same handlers as V1.
    ...
    // "Add a row" buttons of the edit modal's sub-tables.
...
/**
 * The endpoint sends `active` as a real boolean, but older rows and the
 * maintenance flag come through as "Y"/"N" strings. Accept both, like V1 did.
 */
function envV2IsActive(row) {
    return row.active === true || row.active === "Y";
}

/**
 * Yes/no chip.
 *
 * V1 rendered a disabled <input type="checkbox">, which looks like a control the
 * user should be able to click and cannot - and disabled inputs are skipped by
 * keyboard navigation, so the value was invisible to a screen reader too.
 */
function envV2Flag(value) {
    var on = (value === true || value === "Y");
    var cls = on
...
/**
 * Deep-link filters, from the same seven URL parameters V1 read.
 * Keys are SERVER column names - envGp's is inv.gp1, not envGp.
 */
function envV2FiltersFromUrl() {
    var mapping = [
        {param: "system", field: "system"},
        {param: "country", field: "country"},
        {param: "environment", field: "environment"},
        {param: "build", field: "build"},
        {param: "revision", field: "revision"},
        {param: "envgp", field: "inv.gp1"},
        {param: "active", field: "active"}
    ];
    var filters = {};
...
/** Reload the list. Called by the modals after a save. */
function envV2Reload() {
    var table = crbTableInstance(CRB_ENV_TABLE_ID);
    if (table) {
        table.reload();
    }
}
...
/**
 * Delete an environment. Same dialog and same servlet as Environment.js:290-323;
 * only the post-delete refresh changed, because the DataTable it redrew is gone.
 */
async function deleteEntryClick(system, country, environment) {
    ...
    var messageComplete = doc.getDocLabel("page_environment", "message_delete")
        .replace("%SYSTEM%", system).replace("%COUNTRY%", country).replace("%ENV%", environment);
    ...
    const result = await crbConfirmDelete({
        ...
        preConfirm: async () => {
            try {
                const resp = await $.post("DeleteCountryEnvParam",
                    {system: system, country: country, environment: environment}, "json");
...
/* --- Carried over from Environment.js (V1) ---------------------------------
 * Everything below is the V1 code, with one change: the four places that
 * redrew #environmentsTable through fnDraw now call envV2Reload(), because that
 * DataTable does not exist on this page any more. Marked "V2:" where changed.
 * ------------------------------------------------------------------------- */
...
function addEntryModalSaveHandler() {
    ...
    $.when(jqxhr).then(function (data) {
        ...
        if (getAlertType(data.messageType) === 'success') {
            envV2Reload(); // V2: was $("#environmentsTable").dataTable().fnDraw(false)
            showMessage(data);
...
/**
 * V2: renamed from editEntryModalCloseHandler.
 *
 * That name is also declared by include/transversal/Application.html, which
 * modalInclusions.jsp pulls into EVERY page and which is parsed after the head
 * scripts - so its declaration won the global name and this body never ran.
 * Nothing on this page called it either way (editEnvironment.html has no close
 * hook, unlike editLabel.html), so rather than fight over the global name the
 * function is scoped to this page. editEntryClick() below calls it explicitly.
 */
function envV2EditModalCloseHandler() {
    var form = $('#editEnvModal #editEnvModalForm')[0];
    if (form) {
        form.reset();
    }
    $('#editEnvModal').find('div.has-error').removeClass("has-error");
    clearResponseMessage($('#editEnvModal'));
}

function editEntryClick(system, country, environment) {
    clearResponseMessageMainPage();
    // Start from a clean form: the modal is reused across rows, and V1's close
    // handler never ran (see envV2EditModalCloseHandler above), so error styling
    // and the previous environment's leftovers survived from one open to the next.
    envV2EditModalCloseHandler();
...
/** Date cell for the modal tables. An unset date renders blank, not an epoch. */
function envV2Date(value) {
...
/**
 * The two tables inside the edit modal - last changes and last events - are the
 * shared V2 table (js/global/crbTable.js) in EMBEDDED mode: the bordered block
 * around each one is the modal's own, so the component draws no card.
 *
 * They are SERVER-mode tables whose endpoint carries the environment being looked
 * at, and the modal is reopened on a different environment all the time - so the
 * endpoint is re-pointed and the table reloaded, rather than rebuilt. Rebuilding
 * would drop the user's column choices and re-inject markup into a live modal.
 */
function envV2EnsureModalTable(id, mount, endpoint, columns, sort) {
    var existing = crbTableInstance(id);
    if (existing) {
        // Same table, new environment: swap the endpoint and reload.
        existing.cfg.endpoint = endpoint;
        window.crbTableRegistry[id].endpoint = endpoint;
        existing.start = 0;
        existing.fetch();
        return existing;
    }
...
function loadChangeTable(selectSystem, selectCountry, selectEnvironment) {
    var doc = new Doc();
    envV2EnsureModalTable(
        'envChangeTableV2', '#lastChangeList',
        "ReadCountryEnvParam_log?system=" + encodeURIComponent(selectSystem) +
            "&country=" + encodeURIComponent(selectCountry) +
            "&environment=" + encodeURIComponent(selectEnvironment),
        [
            {
                field: 'datecre', title: doc.getDocOnline("countryenvparam_log", "datecre"),
                width: '190px',
                render: function (row) { return crbTableEscape(envV2Date(row.datecre)); }
            },
            {field: 'description', title: doc.getDocOnline("countryenvparam_log", "Description"),
             width: '380px', like: true},
            {field: 'build', title: doc.getDocOnline("buildrevisioninvariant", "versionname01"), width: '130px'},
            {field: 'revision', title: doc.getDocOnline("buildrevisioninvariant", "versionname02"), width: '130px'},
            {field: 'creator', title: doc.getDocOnline("countryenvparam_log", "Creator"), width: '130px'}
        ],
        {field: 'datecre', dir: 'desc'}
    );
}
...
function loadEventTable(selectSystem, selectCountry, selectEnvironment) {
    var doc = new Doc();
    envV2EnsureModalTable(
        'envEventTableV2', '#lastEventList',
        "ReadBuildRevisionBatch?system=" + encodeURIComponent(selectSystem) +
            "&country=" + encodeURIComponent(selectCountry) +
            "&environment=" + encodeURIComponent(selectEnvironment),
        [
            {
                field: 'dateBatch', title: doc.getDocOnline("buildrevisionbatch", "dateBatch"),
                width: '190px',
                render: function (row) { return crbTableEscape(envV2Date(row.dateBatch)); }
            },
            {field: 'batch', title: doc.getDocOnline("buildrevisionbatch", "batch"),
             width: '460px', like: true},
            {field: 'build', title: doc.getDocOnline("buildrevisionbatch", "build"), width: '130px'},
            {field: 'revision', title: doc.getDocOnline("buildrevisionbatch", "revision"), width: '130px'}
        ],
        {field: 'dateBatch', dir: 'desc'}
    );
}
