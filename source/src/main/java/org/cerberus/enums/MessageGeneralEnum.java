...
public enum MessageGeneralEnum {
    ...
    VALIDATION_FAILED_RUNQA_NOTDEFINED(57, "FA", "The test case isn't defined to run in QA environment. You try to run it on '%ENV%' that belong to the QA environment group."),
    VALIDATION_FAILED_RUNUAT_NOTDEFINED(58, "FA", "The test case isn't defined to run in UAT environment. You try to run it on '%ENV%' that belong to the UAT environment group."),
    VALIDATION_FAILED_RUNPROD_NOTDEFINED(59, "FA", "The test case isn't defined to run in Production environment. You try to run it on '%ENV%' that belong to the PROD environment group."),
    ...
    VALIDATION_FAILED_SELENIUM_COULDNOTCONNECT(63, "FA", "Could not contact Selenium server on %SSIP% using port %SSPORT%. Possible causes are invalid address of the remote server or browser start-up failure. %ERROR%"),
    ...
    VALIDATION_FAILED_URL_MALFORMED(86, "FA", "URL to access Selenium server '%URL%' is not correct and cannot be handled by Cerberus. Please check your Selenium server and port parameter."),
    ...
    EXECUTION_FA(3, "FA", "The test case failed to be executed. More lickely due to an error in the test or in Cerberus configuration. %DETAILMESSAGE%"),
