...
public enum MessageEventEnum {
    ...
    PROPERTY_FAILED_GETFROMDATALIB_STATIC_RANDOMNEW_NOMORERECORD(904, "FA", "Data Library entry %ENTRY% (%ENTRYID%) retrieved but there are no more data available. All %TOTNB% entries, has been removed (Already used for the same property name, testcase in the same country environment and build). Data fetched from TestDataLib.", true, false, false, MessageGeneralEnum.EXECUTION_FA),
    PROPERTY_FAILED_GETFROMDATALIB_STATIC_NOTINUSE_NOMORERECORD(904, "FA", "Data Library entry %ENTRY% (%ENTRYID%) retrieved but there are no more data available. All %TOTNB% entries, has been removed (Recent pending (PE) executions for the same property name in the same country, environment). Data fetched from TestDataLib.", true, false, false, MessageGeneralEnum.EXECUTION_FA),
