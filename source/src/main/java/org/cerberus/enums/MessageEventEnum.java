...
public enum MessageEventEnum {
    ...
    PROPERTY_SUCCESS_GETFROMDATALIB_SQL_STATIC(100, "OK", "Data Library entry %ENTRY% (%ENTRYID%) retrieved with success. First result fetch from SQL executed against database '%DB%' and JDBCPOOL '%JDBCPOOLNAME%'. SQL : '%SQL%'", false, false, false, MessageGeneralEnum.EXECUTION_PE_TESTSTARTED),
    ...
    PROPERTY_SUCCESS_GETFROMDATALIB_SQL_RANDOMNEW(100, "OK", "Data Library entry %ENTRY% (%ENTRYID%) retrieved with success. Out of %TOTNB% entries, %REMNB% removed (Already used for the same property name, testcase in the same country environment and build). Random result (position %POS% from %TOTALPOS%) taken. Data fetched from SQL executed against database '%DB%' and JDBCPOOL '%JDBCPOOLNAME%'. SQL : '%SQL%'", false, false, false, MessageGeneralEnum.EXECUTION_PE_TESTSTARTED),
    PROPERTY_SUCCESS_GETFROMDATALIB_SQL_NOTINUSE(100, "OK", "Data Library entry %ENTRY% (%ENTRYID%) retrieved with success. Out of %TOTNB% entries, %REMNB% removed (Recent pending (PE) executions for the same property name in the same country, environment). Random result (position %POS% from %TOTALPOS%) taken. Data fetched from SQL executed against database '%DB%' and JDBCPOOL '%JDBCPOOLNAME%'. SQL : '%SQL%'", false, false, false, MessageGeneralEnum.EXECUTION_PE_TESTSTARTED),
