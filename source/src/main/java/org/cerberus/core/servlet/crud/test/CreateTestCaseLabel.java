                ...
                if (!(resp.isCodeEquals(MessageEventEnum.DATA_OPERATION_OK.getCode()) && resp.getItem() != null)) {
                    ...
                    for (int j = 0; j < myTestList.length; j++) {
                        ...
                        if (!(resp.isCodeEquals(MessageEventEnum.DATA_OPERATION_OK.getCode()) && resp.getItem() != null)) {
                            ...
                            msg.setDescription(msg.getDescription().replace("%ITEM%", OBJECT_NAME)
                                    .replace("%OPERATION%", "Create")
                                    .replace("%REASON%", "Test Case does not exist."));
                            ans.setResultMessage(msg);
                            massErrorCounter++;
        ...
        if (myTestList.length > 1) {
            if (massErrorCounter == myTestList.length) { // All updates are in ERROR.
                ...
                msg.setDescription(msg.getDescription().replace("%ITEM%", OBJECT_NAME)
                        ...
                        .replace("%REASON%", massErrorCounter + " label links(s) out of " + (myTestList.length * myLabelIdList.length) + " failed to be created due to an issue.<br>") + output_message.toString());
                ans.setResultMessage(msg);
