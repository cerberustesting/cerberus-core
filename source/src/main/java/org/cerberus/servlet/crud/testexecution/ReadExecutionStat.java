...
//        units.add("sizeMax");
        ...
        Date fromD = Date.from(ZonedDateTime.now().minusMonths(1).toInstant());
        Date toD = new Date();
                    ...
                    for (String type : types) {
                        for (String unit : units) {
                            ...
                            if (x != -1) {
                                ...
                                point.put("x", new SimpleDateFormat(DATE_FORMAT).format(d));
                                point.put("y", x);
...
//                objectcount.put("application", convertApplicationToJSONObject(a));
        ...
        objectdinst = new JSONArray();
        for (Map.Entry<String, Boolean> entry : unitMap.entrySet()) {
            String key = entry.getKey();
            JSONObject objectcount = new JSONObject();
            objectcount.put("name", key);
            objectdinst.put(objectcount);
